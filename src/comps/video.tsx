...
const VideoBox: FC<VideoBoxProps> = ({
  ...
  const [currentMicId] = useLocalState(state => [state.currentMicId])
  ...
  return (
    <div
      ref={containerRef}
      className={`bg-[#11131aab] h-full w-full relative rounded-md ${
        isSpeaking && currentMicId
          ? 'border-[2px] border-[#2572ED]'
          : 'border-[2px] border-[#11131A]'
      }`}
    >
        ...
        <div className={classes.centerOverlay}>
          ...
          >
            <Persona
              ...
              initialsColor={stream.empty ? '#7E47EB' : undefined}
